...
function useAdminTheme() {
  ...
  const toggle = useCallback(() => {
    const next = !isDark;
    setIsDark(next);
    localStorage.setItem('neyya-admin-theme', next ? 'dark' : 'light');
    if (next) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('neyya-admin-theme');
    const dark = stored !== 'light'; // Default to dark for admin
    setIsDark(dark);
    if (dark) {
      document.documentElement.classList.add('dark');
    } else {
...
export function AdminTopBar() {
  ...
  return (
    <header className="h-14 bg-white/50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6 flex-shrink-0">
      {/* Left: breadcrumb */}
      <div className="text-sm text-gray-500 dark:text-gray-400">
        <span className="text-gray-400 dark:text-gray-500">Admin</span> <span className="text-gray-300 dark:text-gray-600 mx-1">/</span> <span className="text-gray-800 dark:text-gray-200">{breadcrumb}</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={toggleTheme}
          className="rounded-full p-2 text-gray-400 hover:text-gray-200 hover:bg-gray-700 dark:hover:bg-gray-700 transition-colors"
