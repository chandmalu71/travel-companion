...
export class OfflineTripSelector {
  private trips: Map<string, number> = new Map(); // tripId -> storageBytes

  /**
   * Maximum number of trips that can be selected for offline access.
   */
  static readonly MAX_TRIPS = LIMITS.MAX_OFFLINE_TRIPS;

  /**
   * Maximum total offline storage in bytes (500MB).
   */
  static readonly MAX_STORAGE_BYTES = LIMITS.MAX_OFFLINE_STORAGE_BYTES;
   ...
   */
  addTrip(tripId: string, storageBytes: number = 0): boolean {
